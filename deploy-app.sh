    ...
    then
        ...
        then
           ...
           ssh -q ${ENV_USER}@${ENV_HOSTNAME} "sh ${ENV_STOP}"
            ...
            ##undeploy app
            ssh -q ${ENV_USER}@${ENV_HOSTNAME} "cd ${ENV_INSTANCE} && ./jboss-cli.sh --connect controller=${ENV_HOSTNAME}:10190 --command='undeploy ${ARTIFACT}'"
            if [ $? -eq 0 ]
            then
            ...
            ##deploy app
           ssh -q ${ENV_USER}@${ENV_HOSTNAME} "cd ${ENV_INSTANCE} && ./jboss-cli.sh --connect controller=${ENV_HOSTNAME}:10190 --command='deploy ${ENV_DEPLOY}/${ARTIFACT} --force'"
            if [ $? -eq 0 ]
            then
